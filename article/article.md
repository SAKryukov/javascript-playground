...
When you write regular JavaScript code without `new Function`, you cannot catch some errors on lexical level.
...
However, if you enter the second code fragment in `editor` element, text area, pass its value to `evaluate` and sandwich it a try-catch block, the lexical-level problem will be processed as an exception!
...
The idea behing `playgroundAPI` is to pass some script to main JavaScript Playground application "index.html", populate the value of the `editor` control with the script text, and optionally execute it in the main application, optionally in strict mode. In this kind of usage, let's consider main JavaScript Playground application as a "host", used by the "client" part written by the user.
...
The method `userCall` provides the basic functionality of the client part. It initialize the operation. The function `showSample` should use calculate the URL for the host Web page, the content of the second `script` element containing the text of the user script, defines the options and pass this data to `userCall`. This data is stored in the [`sessionStorage`](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage) in the form of [`JSON`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON).

To see how the data is collected from the user-supplied HTML by `showSample`, see its implementation in `playgroundAPI.js`.

After the user HTML to the host Web page, it is loaded and calls `JavaScriptPlaygroundAPI.onLoad`. If the content of `sessionStorage` found by the key `APIDataKey` and evaluates to `true`, and the script text is not empty, `JavaScriptPlaygroundAPI.onLoad` calls the `handler` supplied by the host JavaScript Playground application. Depending on the options passed via JSON content, the host changes the page title, populates the `editor` element, optionally switched to the strict or non-strict mode and optionally executes the user script code.
